...
describe('ApproveTransactionUseCase', () => {
  ...
  describe('execute - success cases', () => {
    ...
    const mockApprovedTransaction = new TransactionEntity(
      'transaction_id',
  ...
  describe('execute - validation error cases', () => {
    ...
    it('should fail when transaction is already approved', async () => {
      const alreadyApprovedTransaction = new TransactionEntity(
      ...
      const result = await useCase.execute({
        id: 'transaction_already_approved',
      });

      expect(result.isFailure()).toBe(true);
      expect(result.error).toBeInstanceOf(TransactionAlreadyProcessedError);
      expect(result.error.message).toBe(
        'Transaction transaction_already_approved is already processed with status: APPROVED',
      );
  ...
  describe('execute - repository error cases', () => {
    ...
    it('should fail when transaction repository throws error on findById', async () => {
      ...
      expect(result.error.message).toContain('Error approving transaction');
